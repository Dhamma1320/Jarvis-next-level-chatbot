...
# Async func to send a query to the Hugging Face API
...
async def generate_images(prompt: str):
    ...
    for i, image_bytes in enumerate(image_bytes_list):
        if image_bytes:  # Only save if API returned valid data
            filename = fr"Data\{safe_prompt}{i + 1}.jpg"
            with open(filename, "wb") as f:
                f.write(image_bytes)
            # Removed "Saved ..." message
        else:
            print(f"Skipping image {i + 1} (no data from API)")
